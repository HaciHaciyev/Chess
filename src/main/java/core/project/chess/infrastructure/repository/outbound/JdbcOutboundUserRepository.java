...
public class JdbcOutboundUserRepository implements OutboundUserRepository {
    ...
    private EmailConfirmationToken userTokenMapper(ResultSet rs, int rowNum) throws SQLException {
        ...
        return EmailConfirmationToken.fromRepository(
                ...
                rs.getBoolean("token_confirmation"), userAccountMapper(rs, rowNum)
        );
    }

    private UserAccount userAccountMapper(ResultSet rs, int rowNum) throws SQLException {
        ...
        return UserAccount.fromRepository(
                UUID.fromString(rs.getString("id")), new Username(rs.getString("username")), new Email(rs.getString("email")),
                new Password(rs.getString("password")), new Rating(rs.getShort("rating")), rs.getBoolean("is_enable"), events
        );
    }
}
